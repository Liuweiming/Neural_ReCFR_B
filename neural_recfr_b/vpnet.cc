...
bool CreateGraphDef(const Game& game, double learning_rate, double weight_decay,
  ...
  return RunPython("neural_recfr_b.export_vpnet", parameters);
}
...
double CFRNetModel::CFRNet::Learn(const std::vector<TrainInputs>& inputs,
  ...
  int num_value_size = inputs[0].value.size() == inputs[0].masks.size()
                           ? num_actions_
                           : num_actions_ + 1;
  ...
  if (!training_batch_size) {
    ...
    for (int b = 0; b < training_batch_size; ++b) {
      ...
      for (int i = 0; i != inputs[b].masks.size(); ++i) {
        targets_matrix(b, inputs[b].masks[i]) = inputs[b].value[i];
      }
