...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  NeuralReCFRBSolver solver(game, value_0_eval, value_1_eval, policy_0_eval,
  ...
  while (!stop->StopRequested()) {
    ...
    if (config.use_tabular) {
      ...
      for (auto& infoset : infosets) {
        ...
        double cfr_value = backward_update_regret(
            Alpha, delta * delta * actions.size(), info_value, eval_steps);
        info_value.push_back(cfr_value);
        std::vector<double> tabular_policy =
            model->GetCFRTabularPolicy({inf_input}, true);
        std::vector<std::string> str_actions(actions.size());
        str_actions.push_back("a");
        absl::c_transform(
...
LearningInfo learn_imp_(const Game& game, const DeepCFRConfig& config,
                        ...
                        CircularBuffer<PolicyReplayNode>& policy_replay_buffer,
  ...
  std::vector<ReplayNode> last_nodes;
  std::vector<double> instant_cfr_values;
    ...
    if (trajectory) {
      ...
      if (value_or_policy) {
        ...
        for (++iter; iter != trajectory->states.end(); ++iter, ++next) {
          ...
          iter->next_information = next->information;
          iter->next_legal_actions = next->legal_actions;
          iter->next_action_index = next->action_index;
          if (iter->next_action_index == -1) {
        ...
        if (trajectory->states.size() > 1) {
          auto& last_node = trajectory->states.back();
          SPIEL_CHECK_GT(last_node.legal_actions.size(), 0);
          last_nodes.emplace_back(last_node);
        } else {
          SPIEL_CHECK_EQ(trajectory->states.size(), 1);
          instant_cfr_values.push_back(trajectory->states.back().value);
        }
      } else {
        for (ReplayNode& n : trajectory->states) {
          SPIEL_CHECK_EQ(n.current_player, player);
          PolicyReplayNode policy_node{n.info_str,       n.information,
                                       n.current_player, n.legal_actions,
                                       n.action_index,   n.weight};
          policy_replay_buffer.Add(policy_node, rng);
          num_states += 1;
        }
      }
      // std::cout << "learn imp " << value_or_policy << " " << player << " "
      //           << trajectory->states.size() << std::endl;
  ...
  if (true) {
    ...
    if (value_or_policy) {
      std::vector<CFRNetModel::InferenceInputs> last_inputs;
      for (auto& node : last_nodes) {
        last_inputs.emplace_back(CFRNetModel::InferenceInputs{
            node.info_str, node.legal_actions, node.information});
      }
      std::vector<CFRNetModel::InferenceOutputs> last_outputs =
          learn_model->InfValue(player, last_inputs);
      for (int i = 0; i != last_nodes.size(); ++i) {
        SPIEL_CHECK_GT(last_outputs[i].value.size(), 1);
        double exp_value = backward_update_regret(
            alpha, last_nodes[i].opponent_reach, last_outputs[i].value, step);
        instant_cfr_values.push_back(exp_value);
      }

      double value = std::accumulate(instant_cfr_values.begin(),
                                     instant_cfr_values.end(), 0.0) /
                     config.cfr_batch_size;
      cfr_value[player] = value;
      alpha = adapt(config, alpha, cfr_value);
      Alpha = alpha;
      // if (step % 1 == 0) {
      //   logger.Print("alpha: %.3f, value: %.3f, cfr_value: [%.3f, %.3f]",
      //   alpha,
      //                value, cfr_value[0], cfr_value[1]);
      //   std::cout << "alpha" << alpha << " " << value << cfr_value <<
      //   std::endl;
      // }
    }
    ...
    //   train_steps = replay_buffer.Size() * train_steps / train_batch_size;
    //   if (step % 1000 == 0) {
      ...
      if (value_or_policy) {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          target_inference_inputs.emplace_back(CFRNetModel::InferenceInputs{
              node.info_str, node.next_legal_actions, node.next_information});
        }
        target_outputs =
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          if (node.next_action_index != -1) {
            SPIEL_CHECK_FALSE(node.next_legal_actions.empty());
            SPIEL_CHECK_FALSE(target_outputs[i].value.empty());
            std::vector<double>& ref_value = target_outputs[i].value;
            target_value[0] = backward_update_regret(alpha, node.opponent_reach,
                                                     ref_value, step);
          }
          target_value[0] += node.value;
        ...
        std::vector<PolicyReplayNode> data =
            policy_replay_buffer.Sample(rng, train_batch_size);
        for (int i = 0; i != data.size(); ++i) {
          const auto& node = data[i];
          std::vector<double> value(node.legal_actions.size(), 0);
          value[node.action_index] = 1;
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
              node.info_str, node.legal_actions, node.legal_actions,
              node.information, value, node.weight});
        }
      }
      {
        auto start = std::chrono::system_clock::now();
...
void learner(const Game& game, const DeepCFRConfig& config,
  ...
  std::vector<ReserviorBuffer<PolicyReplayNode>> policy_replay_buffer(
      2, ReserviorBuffer<PolicyReplayNode>(config.policy_memory_size));
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  if (config.graph_def.empty()) {
    config.graph_def =
        absl::StrJoin({config.game, std::string("neural_recfr_b")}, "_");
  ...
  for (int i = 0; i < config.num_gpus; ++i) {
    device_manager.AddDevice(CFRNetModel(
        *game, config.path, model_path, config.graph_def, config.omp_threads,
        ...
        config.use_policy_net, true, true));
    if (config.num_cpus) {
